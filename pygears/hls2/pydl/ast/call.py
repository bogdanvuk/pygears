...
import typing
import inspect
from . import Context, FuncContext, Function, node_visitor, nodes, visit_ast
...
def const_func_args(args, kwds):
    return (all(isinstance(node, nodes.ResExpr) for node in args)
            and all(isinstance(node, nodes.ResExpr) for node in kwds.values()))
...
def resolve_gear_cal(func, args, kwds):
    ...
    for f, args, templates in get_gear_signatures(func, args, kwds):
        try:
            ...
            return nodes.GenCallExpr(f, args, kwds, params)
