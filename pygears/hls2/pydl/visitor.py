...
class PydlExprRewriter:
    ...
    def visit_ConcatExpr(self, node):
        return nodes.ConcatExpr(tuple(op for op in node.operands))
    ...
    def visit_BinOpExpr(self, node):
        return nodes.BinOpExpr(tuple(op for op in node.operands), node.operator)

    def visit_SubscriptExpr(self, node):
        return nodes.SubscriptExpr(self.visit(node.val), self.visit(node.index))

    def visit_ConditionalExpr(self, node):
        return nodes.ConditionalExpr(self.visit(node.cond), tuple(op for op in node.operands))
