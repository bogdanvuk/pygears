...
Why am I placing my code at address ``0x80000000``? Because nothing else worked. My best guess is that simulator maps RAM at address ``0x80000000`` by default and gets angry if you want your code somewhere else. I created proof of concept assembly file ``hello.s`` which contains the example instruction that I want to test ``li a1, 1`` and some boilerplate needed to play nicely with the Spike simulator:
...
Sucess! The target test instruction is first to be executed, which will simplify my tests. I can now invoke Spike simulator for the basic 32-bit ISA (``--isa=rv32i`` option) to test the instruction execution and print the list of the instructions it their execution order (``-l`` option):

.. code-block:: bash

   spike -l --isa=rv32i hello

Log shows that the simulator inserted 5 additional instructions at address ``0x1000``, which I guess is the fixed position where the execution starts. Last of these five jumps to my example test instruction, now at address ``0xffffffff80000000``? Sign extension I guess?
