    ...
    for (i = 0; i < tmp.length; i++) {
        if (typeof tmp[i] === 'object') {
            if (Object.prototype.toString.call(tmp[i]) === '[object Array]') {
                old.y = state.y;
                state = rec(tmp[i], state, lvl+1);
                state.groups.push({'x':state.xx, 'y':old.y, 'height':(state.y - old.y), 'name':state.name, 'lvl': lvl});
            } else {
...
function renderGroups (groups, index, lane) {
    ...
    groups.forEach(function (e, i) {
        if (e.lvl == 0) {
            glengths.push(textWidth(e.name, 11));
        }
    });

    xmax = Math.max.apply(Math, glengths);

    groups.forEach(function (e, i) {
        res.push(['path',
            {
                id: 'group_' + i + '_' + index,
                d: ('m ' + (e.x + xmax + 0.5) + ',' + (e.y * lane.yo + 3.5 + lane.yh0 + lane.yh1)
        ...
        if (e.lvl == 0) {
            ...
            res.push(['g', {transform: 'translate(' + x + ',' + y + ')'}, ['g', {}, ts]]);
        } else {
            ts.unshift(
                'text',
                {
            ...
            res.push(['g', {transform: 'translate(' + x + xmax + ',' + y + ')'}, ['g', {transform: 'rotate(270)'}, ts]]);
        }
    });
...
},{"./text-width.js":32,"tspan":37}],24:[function(require,module,exports){
'use strict';
...
function renderSignal (index, source, waveSkin) {
    ...
    lane.xg = Math.ceil((xmax - lane.tgo) / lane.xs) * lane.xs + Math.max.apply(Math,waveGroups.glengths);
